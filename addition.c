...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (bytes_read != sizeof(float)) {
            // Okuma sırasında hata oluşursa hata mesajını yazdır
        ...
        // İşlemi gerçekleştir (toplama işlemi)
        ...
        // Yeni bir süreç oluştur ve sonucu kaydetmek için saver programını çalıştır
        int pid = fork();
        if (pid == -1) {
            // Süreç oluşturma sırasında hata oluşursa hata mesajını yazdır
